...
lm_model_dir = osp.normpath(osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/lmo/models"))
model_paths = [osp.join(lm_model_dir, f"obj_{obj_id:06d}.ply") for obj_id in cls_indexes]
texture_paths = None

scenes = [i for i in range(0, 49 + 1)]
xyz_root = osp.normpath(osp.join(PROJ_ROOT, "datasets/BOP_DATASETS/lmo/train_pbr/xyz_crop"))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="gen lmo train_pbr xyz")
    ...
    setproctitle.setproctitle(f"gen_xyz_lmo_train_pbr_{args.split}_{args.scene}")
